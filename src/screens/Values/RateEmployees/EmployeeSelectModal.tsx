  ...
  return (
    visible && (
      ...
      >
        ...
        >
          <h3 className="text-xl font-bold text-center text-gray-800 mb-6">Select your employee</h3>
          <ul className="list-none p-0 m-0">
            {employees.map((item) => (
              <li key={item._id} className="flex justify-between py-2 border-b border-gray-200">
                <span className="text-lg font-normal text-black">{item.fullName}</span>
                <button
                  className={`w-5 h-5 rounded-full border-2 ${tempSelectedEmployee?.fullName === item.fullName ? 'bg-blue-500 border-blue-500' : 'bg-transparent border-gray-300'}`}
          ...
          <button
            className={`w-full py-4 text-white font-bold text-lg rounded-lg mt-6 ${tempSelectedEmployee ? 'bg-blue-500' : 'bg-gray-400'}`}
