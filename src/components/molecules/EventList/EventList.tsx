  ...
  return (
    <>
      <div className="flex justify-center gap-4 bg-gray-200 rounded-lg p-1 w-56 mx-auto mt-5 mb-5">
        <div
          className={`flex-1 text-center py-2 px-1 cursor-pointer rounded-md ${
            tab === 'upcoming' ? 'bg-white shadow' : ''
          }`}
          onClick={() => setTab('upcoming')}
        >
          <p className={`text-xs ${tab === 'upcoming' ? 'font-semibold text-gray-900' : 'text-gray-600'}`}>Upcoming Events</p>
        </div>
        <div
          className={`flex-1 text-center py-1 py-2 cursor-pointer justify-center rounded-md ${
            tab === 'past' ? 'bg-white shadow' : ''
          }`}
          onClick={() => setTab('past')}
        >
          <p className={`text-xs ${tab === 'past' ? 'font-semibold text-gray-900' : 'text-gray-600'}`}>Past</p>
        </div>
      </div>
